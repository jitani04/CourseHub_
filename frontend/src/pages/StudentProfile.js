...
import { BookOpen, User, School, GraduationCap, Award, Calendar as CalendarIcon, ChevronLeft, ChevronRight, X, Check, ImagePlus } from "lucide-react";
...
const topics = [
  ...
  "Environmental Science", "Political Science", "Linguistics", "Anthropology", "Engineering", "Pre-Med",
];
...
function StudentProfile() {
  ...
  const [selectedCategory, setSelectedCategory] = useState(diceBearCategories[0]);

  useEffect(() => {
    // Fetch schools and majors from API (replace with actual API calls)
    setSchools(['CSULB', 'CSULA', 'UCLA', 'UCI']);
    setMajors(['Computer Science', 'Engineering', 'Business', 'Psychology', 'Biology']);
  }, []);
                        ...
                        <div className="flex-shrink-0">
                          <div className="relative group">
                            <Avatar className={cn(
                              ...
                              (avatar || customAvatar) && "ring-4 ring-green-500 ring-offset-4"
                            )}>
                              {avatar ? (
                                ...
                                <ScrollArea className="h-48">
                                  <div className="grid grid-cols-5 gap-4">
                                    {Array.from({ length: 15 }, (_, i) => {
                                      const avatarUrl = generateAvatarUrl(category, `avatar-${i}`);
                                      ...
                                      return (
                                        <button
                                          ...
                                          className={cn(
                                            ...
                                            isSelected ? "ring-2 ring-green-500 ring-offset-2" : "hover:ring-2 hover:ring-green-300 hover:ring-offset-2"
                                          )}
                                        >
                                          <Avatar className="w-12 h-12">
                                            <AvatarImage src={avatarUrl} alt={`${category} avatar ${i + 1}`} />
                    ...
                    <CardContent>
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                        {topics.map((topic) => (
                          ...
                          >
                            {topic}
                            {selectedTopics.includes(topic) && <Check className="inline-block ml-2 h-4 w-4" />}
                          </button>
                        ))}
                      </div>
                    </CardContent>
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-6">
                      ...
                      {awards.map((award, index) => (
                        <div
                          key={index}
                          onClick={() => handleListItemRemove(index, setAwards)} // Handle the click to remove the item
                        ...
                        >
                          ...
                          <span className="absolute right-2 inset-y-0 flex items-center ml-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                            <X className="h-4 w-4 font-bold" /> {/* Bold X icon */}
                          </span>
                        </div>
                      ))}
                    </div>
                    ...
                    <div>
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        {clubs.map((club, index) => (
                          <div
                            key={index}
                            onClick={() => handleListItemRemove(index, setClubs)} // Handle the click to remove the item
                            className="relative inline-flex items-center text-sm py-1 pl-2 pr-6 bg-green-100 text-green-800 rounded-lg cursor-pointer transition-all hover:bg-red-100 hover:text-red-600 group"
                          >
                            <span>{club}</span>
                            <span className="absolute right-2 inset-y-0 flex items-center ml-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    ...
                    <div>
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        {certifications.map((cert, index) => (
                          <div
                            key={index}
                            onClick={() => handleListItemRemove(index, setCertifications)} // Handle the click to remove the item
                            className="relative inline-flex items-center text-sm py-1 pl-2 pr-6 bg-green-100 text-green-800 rounded-lg cursor-pointer transition-all hover:bg-red-100 hover:text-red-600 group"
                          >
                            <span>{cert}</span>
                            <span className="absolute right-2 inset-y-0 flex items-center ml-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
