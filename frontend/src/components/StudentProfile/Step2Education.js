  ...
  const fetchMajors = useCallback(async (schoolId) => {
    try {
      ...
      const majorsSet = new Set(
        data.results[0]["latest.programs.cip_4_digit"].map(
                  ...
                  <ul className="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                    {schools.map((schoolItem) => (
                      <li
                        ...
                        onClick={() =>
                          handleSchoolSelect(schoolItem.id, schoolItem.name)
                        }
                      >
                        {schoolItem.name}
                      </li>
          ...
          {school && (
            <div>
              ...
              <div className="relative mt-1">
                ...
                {majors.length > 0 &&
                  majorSearchTerm &&
                  majorSearchTerm !== major && (
                    <ul className="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                      {majors
                        .filter((m) =>
                          m
                            .toLowerCase()
                            .includes(majorSearchTerm.toLowerCase()),
                        )
                        .map((m, index) => (
                          <li
                            key={index}
